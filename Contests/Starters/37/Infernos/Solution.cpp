...
void solve()
{
    ...
    {
        ll t; scan(t);
        res+= (t/x)+(t%x!=0); //for multi target mode
        maxi = max(t,maxi);//for single target mode, the max time required to finish all enemies would 
        //obviously be equal to the maximum power of any enemy
    }
    cout<< min(maxi,res)<<endl;
}
