...
ll dfs(ll v, ll n, vector<unordered_set<ll>>& adj)
{
    if(v==n)
        return 0;
    ...
    for(auto a: adj[v])
        ans = min(ans,dfs(a,n,adj));
    
    for(ll i = 1; i<=n; i++)
        if(adj[v].find(i)==adj[v].end())
            ans = min( ans, ((long long)pow(abs(v-i),2)+dfs(i,n,adj)) );
...
void solve()
{
    ...
    for(ll i = 0; i<m ; i++)
        {ll a,b; scan(a); scan(b); adj[a].insert(b); adj[b].insert(a);}
